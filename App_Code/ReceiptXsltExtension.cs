    ...
    private void PreComputeLineItemIntrinsics(XPathNavigator nav)
    {
        ...
        while (lineItemNodeIterator.MoveNext())
        {
            ...
            if (applyVat && showPriceVatInclusive)
            {
                ...
                if (discount != 0)
                {
                    decimal missingPriceForAttributes = discount; //discount is catching any differences, which is wrong, the differences are VAT on attribute prices
                    if (missingPriceForAttributes > 0)
                        missingPriceForAttributes = missingPriceForAttributes * -1;
                    displayExtPrice += missingPriceForAttributes * taxRate / 100;
                }
            }
