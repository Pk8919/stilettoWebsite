                    ...
                    foreach (RouteElement element in config.Routes)
                    {
                        ...
                        if (element.Url.Contains("-{SEName}"))
                        {
                            ...
                            for (int c = 30; c >= 0; c--)
                            {
                                ...
                                RouteTable.Routes.Add(element.Name + c.ToString(), new Route(Url, new WebFormRouteHandler(element.VirtualPath, element.CheckPhysicalUrlAccess)));
                               
                                
                                Url = Url.Replace("-{SEPart" + c.ToString() + "}", string.Empty);
                            }
                        }
                        RouteTable.Routes.Add(element.Name, new Route(element.Url, new WebFormRouteHandler(element.VirtualPath, element.CheckPhysicalUrlAccess)));
                    }
                }
