...
<xsl:stylesheet version="1.0"
        ...
        <p>
          <xsl:value-of select="aspdnsf:GetVariantPrice(VariantID, HidePriceUntilCart, Price, SalePrice, ExtendedPrice, 0, SalesPromptName)" disable-output-escaping="yes" />
				...
				<xsl:if test="$DisplayOutOfStockOnEntityPages = 'true'">
					<xsl:call-template name="StockMessage">
  ...
  <xsl:template name="StockMessage">
    <xsl:param name="Inventory" />
		
		<xsl:choose>
      <xsl:when test="$Inventory > $OutOfStockThreshold">
        <div class="productListItemInStockHint">
          <xsl:value-of select="aspdnsf:StringResource('OutofStock.DisplayInStockOnEntityPage')" disable-output-escaping="yes"/>
        </div>
      </xsl:when>
      <xsl:otherwise>
        <div class="productListItemOutOfStockHint">
          <xsl:value-of select="aspdnsf:StringResource('OutofStock.DisplayOutOfStockOnEntityPage')" disable-output-escaping="yes"/>
        </div>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
