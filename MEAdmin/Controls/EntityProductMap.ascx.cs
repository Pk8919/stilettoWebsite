...
namespace AspDotNetStorefrontAdmin.Controls
{
    public partial class EntityProductMap : BaseUserControl<IList<ProductEntityMapInfo>>
    {
        ...
        private void SaveProductEntityInfo(ProductEntityMapInfo productMapInfo)
        {
            ...
            {
				if (this.EntityType.ToLower() == "manufacturer" || this.EntityType.ToLower() == "distributor")
					DB.ExecuteSQL("delete from {0} where ProductId = {1}".FormatWith(entityMapTableName, productMapInfo.ProductId));
					
                if (DB.GetSqlN("select count(*) as N from {0} where {1} = {2} and ProductId = {3}".FormatWith(entityMapTableName, entityIdName, productMapInfo.EntityId, productMapInfo.ProductId)) == 0)
                    DB.ExecuteSQL("insert into {0} (ProductId, {1}, DisplayOrder, CreatedOn) values ({2}, {3}, 0, getdate())".FormatWith(entityMapTableName, entityIdName, productMapInfo.ProductId, productMapInfo.EntityId));
            }
            else
            {
                DB.ExecuteSQL("delete from {0} where {1} = {2} and ProductId = {3}".FormatWith(entityMapTableName, entityIdName, productMapInfo.EntityId, productMapInfo.ProductId));
            }
        }
        ...
        protected void DeleteProduct(Int32 productId)
        {
            ...
            DB.ExecuteSQL("delete from customcart where productid=" + productId.ToString());
