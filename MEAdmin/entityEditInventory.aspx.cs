...
namespace AspDotNetStorefrontAdmin
{
    ...
    public partial class entityEditInventory : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
            ...
            LoadData(CommonLogic.FormBool("IsSubmit"));
        }

        protected void LoadData(bool isSubmit)
        {
            ...
            using (SqlConnection dbconn = new SqlConnection(DB.GetDBConn()))
            {
                ...
                using(IDataReader rs = DB.GetRS("select * from productvariant   with (NOLOCK)  where VariantID=" + VariantID.ToString(),dbconn))
                {
                    ...
                    if (ErrorMsg.Length == 0)
                    {
                        ...
                        String Sizes = DB.RSFieldByLocale(rs, "Sizes", Localization.GetDefaultLocale()).Trim();
                        String Colors = DB.RSFieldByLocale(rs, "Colors", Localization.GetDefaultLocale()).Trim();
                        ...
                        if (!isSubmit)
                        {
                            CleanupVariantInventory(ColorsSplit, SizesSplit);
                        }
        ...
        protected void CleanupVariantInventory(String[] NewColors, String[] NewSizes)
        {
            List<string> variantList = new List<string>();
            String tempSize = string.Empty;
            String tempColor = string.Empty;
            String tempSizeAndColor = string.Empty;
            int tempInvID = 0;

            //Build a list of Size/Color combinations that exist on the variant now
            for (int i = NewSizes.GetLowerBound(0); i <= NewSizes.GetUpperBound(0); i++)
            {
                for (int j = NewColors.GetLowerBound(0); j <= NewColors.GetUpperBound(0); j++)
                {
                    variantList.Add(NewSizes[i] + NewColors[j]);
                }
            }
            ...
            {
                conn.Open();
                using (IDataReader rs2 = DB.GetRS("Select InventoryID, Color, Size from Inventory with (NOLOCK) where VariantID=" + VariantID.ToString(), conn))
                {
                    while (rs2.Read())
                    {
                        tempInvID = DB.RSFieldInt(rs2, "InventoryID");
                        tempSize = DB.RSFieldByLocale(rs2, "Size", ThisCustomer.LocaleSetting);
                        tempColor = DB.RSFieldByLocale(rs2, "Color", ThisCustomer.LocaleSetting);
                        tempSizeAndColor = tempSize + tempColor;

                        if(variantList.Contains(tempSizeAndColor))
                        {
                            //Everything's fine, this size/color combination still exists
                        }
                        else
                        {
                            //Size/Color combination no longer exists - nuke it!
                            DB.ExecuteSQL("DELETE FROM Inventory WHERE InventoryID = " + tempInvID);
                        }
                    }
                }
            }
