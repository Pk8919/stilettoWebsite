        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (!GWActual.RecurringIntervalSupportsMultiplier)
                {
                    ...
                    txtRecurringIntervalMultiplier.Visible = false;
                }

                ddRecurringInterval.Items.Clear();
                //allow payflowpro to use it's outragous recurring flow.
                ...
                ltNormalSubscription.Visible = !GWActual.RecurringIntervalSupportsMultiplier;
                ltSpecialSubscription.Visible = GWActual.RecurringIntervalSupportsMultiplier;
                txtSubscriptionInterval.Visible = !GWActual.RecurringIntervalSupportsMultiplier;
                rblSubscription.Visible = GWActual.RecurringIntervalSupportsMultiplier;
            ...
            using (SqlConnection dbconn = new SqlConnection(DB.GetDBConn()))
            {
                ...
                using (IDataReader rs = DB.GetRS("select PV.*,P.ColorOptionPrompt,P.SizeOptionPrompt from Product P  with (NOLOCK)  , productvariant PV   with (NOLOCK)  where PV.ProductID=P.ProductID and VariantID=" + vID.ToString(), dbconn))
                {
                    ...
                    if (Editing)
                    {
                        ...
                        txtWeight.Text = Localization.ParseLocaleDecimal(DB.RSFieldDecimal(rs, "Weight").ToString(), ThisCustomer.LocaleSetting).ToString();
        ...
        protected Boolean UpdateVariant()
        {
            ...
            else
            {
                ...
                sql.Append("Colors=" + DB.SQuote(AppLogic.FormLocaleXmlVariant("Colors",
                    XmlCommon.GetLocaleEntry(Security.HtmlEncode(VariantColors), locale, false),
                ...
                sql.Append("Sizes=" + DB.SQuote(AppLogic.FormLocaleXmlVariant("Sizes",
                    XmlCommon.GetLocaleEntry(Security.HtmlEncode(VariantSizes), locale, false),
                    locale, vID).Replace(", ", ",").Replace(" ,", ",").Replace("'", "").Trim()) + ",");
