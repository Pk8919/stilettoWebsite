    ...
    public partial class mailingTest : AdminPageBase
    {
        ...
        private void LoadContent(Int32 storeId)
        {
            ...
            foreach (String xmlPackage in xmlPackages)
            {
                ...
                if (AppLogic.ProductIsMLExpress() && (xmlPackage.IndexOf("distributor") != -1 || xmlPackage.IndexOf("giftcard") != -1))
                {
                    ddXmlPackageReceipt.Items.Remove(xmlPackage);
                    ddXmlPackageOrderNotifications.Items.Remove(xmlPackage);
                    ddXmlPackageShipped.Items.Remove(xmlPackage);
                }
            }
