                        ...
                        using (IDataReader rs2 = DB.GetRS(statistical, dbconn2))
                        {
                            if (rs2.Read() && DB.RSFieldInt(rs2, "N") > 0)
                            {
                                if (rs2.NextResult())
                                {
                                    ...
                                    while (rs2.Read())
                                    {
                                        ltReport.Text += "<tr class='reportTitle' style=\"cursor:hand; cursor:pointer\"><td><!--Category: -->" + DB.RSFieldByLocale(rs2, "CategoryName", cust.LocaleSetting) + "</td><td>" + String.Format(AppLogic.GetString("admin.orderReports.Total", SkinID, LocaleSetting),cust.CurrencyString(DB.RSFieldDecimal(rs2, "Revenue"))) + "</td><td>" + String.Format(AppLogic.GetString("admin.orderReports.Quantity", SkinID, LocaleSetting),DB.RSFieldInt(rs2, "Total")) + "</td></tr>";
