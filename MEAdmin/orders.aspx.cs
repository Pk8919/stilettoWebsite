...
namespace AspDotNetStorefrontAdmin
{
    ...
    public partial class orders : AdminPageBase
    {
        ...
        public string WhereClause()
        {
            ...
            {
                result += string.Format(" and OrderNumber in (select distinct ordernumber from orders_shoppingcart where OrderedProductSKU = {0}) ", DB.SQuote(txtProductSKU.Text.Trim()));
            }
            ...
            if (!string.IsNullOrEmpty(txtPriceRangeHigh.Text) && !string.IsNullOrEmpty(txtPriceRangeLow.Text))
            {
                ...
                if (Decimal.TryParse(txtPriceRangeHigh.Text.Replace("$", "").Trim(), out highprice) && Decimal.TryParse(txtPriceRangeLow.Text.Replace("$", "").Trim(), out lowprice))
                {
                    ...
                    result += string.Format(" and ordertotal >= {0} and ordertotal <= {1} ", lowprice, highprice);
                }
            }
            ...
            if (ddPaymentMethod.SelectedValue != "-")
            {
                ...
                if (PM == "GOOGLECHECKOUT") //not defined in AppLogic
                {
                    result += String.Format(" and (PaymentGateway is not null and PaymentGateway LIKE 'Google%')");
                }
                if (PM == "CHECKOUTBYAMAZON") //not defined in AppLogic
                {
                    result += String.Format(" and (PaymentMethod = {0})", DB.SQuote(ddPaymentMethod.SelectedValue));
                }
