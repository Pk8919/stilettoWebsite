...
namespace AspDotNetStorefrontAdmin
{
    ...
    public partial class shippingfrm : AdminPageBase
    {
        ...
        private void InitializeStores()
        {
            ...
            int DefaultStoreID = defStore.StoreID;

            if (!MultiStoreFilteringEnabled)
            {
                StoreFilter = DefaultStoreID;
            }
            else
            {
            ...
            //if ((!IsPostBack && CommonLogic.FormNativeInt("StoreFilter") == 0) || Stores.Count == 1)
            //{
            ...
            //    }
            //}
            //else if (MultiStoreFilteringEnabled)
            //{
            //    if (IsStoreFilterChangePostBack())
            //    {
            //        StoreFilter = CommonLogic.FormNativeInt("cboStores");
            //    }
            //    else if (CommonLogic.FormNativeInt("StoreFilter") > 0)
            //    {
        ...
        {
            string Check = Request["__EVENTTARGET"];
        ...
        private void Render()
        {
            ...
            if (CommonLogic.FormBool("IsSubmitFixedRate"))
            {
                ...
                {
                    con.Open();
                    string queryShippingMethod = string.Format("exec aspdnsf_GetStoreShippingMethodMapping  @StoreID = {0}, @IsRTShipping=0, @OnlyMapped = 1, @ExcludeNameLike = 'RealTime'", StoreFilter);
                    using (IDataReader rs = DB.GetRS(queryShippingMethod, con))
                    {
                        while (rs.Read())
                        {
            ...
            if (CommonLogic.FormBool("IsSubmitFixedPercentOfTotal"))
            {
                ...
                {
                    con.Open();
                    //using (IDataReader rs = DB.GetRS("select * from ShippingMethod   with (NOLOCK)  where name not like " + DB.SQuote("%Real Time%") + " and StoreId=" + StoreFilter.ToString() + " and IsRTShipping=0 order by DisplayOrder", con))
                    string queryShippingMethod = string.Format("exec aspdnsf_GetStoreShippingMethodMapping  @StoreID = {0}, @IsRTShipping=0, @OnlyMapped = 1, @ExcludeNameLike = 'RealTime'", StoreFilter);
                    using (IDataReader rs = DB.GetRS(queryShippingMethod, con))
                    {
                        while (rs.Read())
            ...
            if (CommonLogic.FormBool("IsSubmitByTotal"))
            {
                ...
                if (Low0 != System.Decimal.Zero || High0 != System.Decimal.Zero)
                {
                    ...
                    {
                        con.Open();
                        //using (IDataReader rs = DB.GetRS("select * from ShippingMethod with (NOLOCK)  where StoreId=" + StoreFilter.ToString() + " and IsRTShipping=0 order by DisplayOrder", con))
                        string queryShippingMethod = string.Format("exec aspdnsf_GetStoreShippingMethodMapping  @StoreID = {0}, @IsRTShipping=0, @OnlyMapped = 1, @ExcludeNameLike = 'RealTime'", StoreFilter);
                        using (IDataReader rs = DB.GetRS(queryShippingMethod, con))
                        {
                            while (rs.Read())
                ...
                if (Low0 != 0.0M || High0 != 0.0M)
                {
                    ...
                    using (SqlConnection con = new SqlConnection(DB.GetDBConn()))
                    {
                        ...
                        using (IDataReader rs = DB.GetRS(queryShippingMethod, con))
                        {
                            ...
                            {
                                decimal Charge = CommonLogic.FormUSDecimal("Rate_0_" + DB.RSFieldInt(rs, "ShippingMethodID").ToString());
                                DB.ExecuteSQL("insert into ShippingByWeight(RowGUID,LowValue,HighValue,ShippingMethodID,ShippingCharge,StoreID) values(" + DB.SQuote(NewRowGUID) + "," + Localization.DecimalStringForDB(Low0) + "," + Localization.DecimalStringForDB(High0) + "," + DB.RSFieldInt(rs, "ShippingMethodID").ToString() + "," + Localization.CurrencyStringForDBWithoutExchangeRate(Charge) + ", "+StoreFilter+")");
                            }
                        }
                    }
            ...
            html.Append("<form action=\"" + AppLogic.AdminLinkUrl("shipping.aspx") + "\" method=\"post\" id=\"ShippingForm\" name=\"aspnetForm\" onsubmit=\"return (validateForm(this) && ShippingForm_Validator(this))\" onReset=\"return confirm('Do you want to reset all fields to their starting values?');\">\n");
            ...
            if (Stores.Count > 1)
            {
                ...
                html.AppendLine();
                html.AppendFormat("Store: <select id=\"cboStores\" name=\"cboStores\" "+CommonLogic.IIF(MultiStoreFilteringEnabled, "", "disabled=\"disabled\"")+" onchange=\"javascript:setTimeout('__doPostBack(\\'cboStores\\',\\'\\')', 0)\" >\n");
            ...
            //litContents.Text = html.ToString();
        ...
        private void RenderCalculateShippingByWeightForm(StringBuilder html, String EditGUID)
        {
            ...
            html.Append("<form action=\"" + AppLogic.AdminLinkUrl("shipping.aspx") + "?EditGUID=" + EditGUID + "\" method=\"post\" id=\"ByWeightForm\" name=\"ByWeightForm\" onsubmit=\"return (validateForm(this) && ByWeightForm_Validator(this))\" onReset=\"return confirm('Do you want to reset all fields to their starting values?');\">\n");
            ...
            using (DataTable dtWeight = new DataTable())
            {
                ...
                {
                    con.Open();
                    //string queryShippingMethod = string.Format("select * from ShippingMethod   with (NOLOCK)  where StoreID = {0} and IsRTShipping=0 order by DisplayOrder", StoreFilter);
                    string queryShippingMethod = string.Format("exec aspdnsf_GetStoreShippingMethodMapping  @StoreID = {0}, @IsRTShipping=0, @OnlyMapped = 0", StoreFilter);
                    using (IDataReader rs = DB.GetRS(queryShippingMethod, con))
                    {
                        dtWeight.Load(rs);
                    }
                ...
                if (dtWeight.Rows.Count > 0)
                {
                    ...
                    //if (StoreFilter == Shipping.DONT_FILTER_PER_STORE)
                    //{
...
//                                        FROM ShippingByWeight sw WITH (NOLOCK) 
//                                        INNER JOIN ShippingMethod sm WITH (NOLOCK) ON sm.ShippingMethodid = sw.ShippingMethodId 
//                                        INNER JOIN ShippingMethodStore ssm WITH (NOLOCK) ON ssm.StoreID = sm.StoreID
...
//                    }
                    ...
                    using (SqlConnection con = new SqlConnection(DB.GetDBConn()))
                    {
                        ...
                        using (IDataReader rs = DB.GetRS(sqlShipping, con))
                        {
                            while (rs.Read())
                            {
                                ...
                                {
                                    html.Append("<td align=\"left\" valign=\"middle\"><input class=\"normalButtons\" type=\"Button\" name=\"Edit\" value=\"Edit\" onClick=\"self.location='" + AppLogic.AdminLinkUrl("shipping.aspx") + "?EditGUID=" + DB.RSFieldGUID(rs, "RowGUID") + "'\"></td>\n");
                                }
                                html.Append("<td align=\"left\" valign=\"middle\"><input class=\"normalButtons\" type=\"Button\" name=\"Delete\" value=\"Delete\" onClick=\"self.location='" + AppLogic.AdminLinkUrl("shipping.aspx") + "?DeleteByWeightID=" + DB.RSFieldGUID(rs, "RowGUID") + "'\"></td>\n");
                                html.Append("</tr>\n");
                            }
                        }
                    }
        ...
        private void RenderCalculateShippingByTotalForm(StringBuilder html, String EditGUID)
        {
            ...
            html.Append("<form action=\"" + AppLogic.AdminLinkUrl("shipping.aspx") + "?EditGUID=" + EditGUID + "\" method=\"post\" id=\"ByTotalForm\" name=\"ByTotalForm\" onsubmit=\"return (validateForm(this) && ByTotalForm_Validator(this))\" onReset=\"return confirm('Do you want to reset all fields to their starting values?');\">\n");
            ...
            using (DataTable dtTotal = new DataTable())
            {
                using (SqlConnection con = new SqlConnection(DB.GetDBConn()))
                {
                    con.Open();
                    //string queryShippingMethod = string.Format("select * from ShippingMethod   with (NOLOCK)  where StoreID = {0} and IsRTShipping=0 order by DisplayOrder", StoreFilter);
                    string queryShippingMethod = string.Format("exec aspdnsf_GetStoreShippingMethodMapping  @StoreID = {0}, @IsRTShipping=0, @OnlyMapped = 1", StoreFilter);
                    using (IDataReader rs = DB.GetRS(queryShippingMethod, con))
                    {
                        dtTotal.Load(rs);
                    }
                    ...
                    using (SqlConnection con = new SqlConnection(DB.GetDBConn()))
                    {
                        ...
                        using (IDataReader rs = DB.GetRS(sqlShipping, con))
                        {
                            while (rs.Read())
                            {
                                ...
                                html.Append("<td align=\"left\" valign=\"middle\"><input class=\"normalButtons\" type=\"Button\" name=\"Delete\" value=\"Delete\" onClick=\"self.location='" + AppLogic.AdminLinkUrl("shipping.aspx") + "?deleteByTotalid=" + DB.RSFieldGUID(rs, "RowGUID") + "'\"></td>\n");
                                html.Append("</tr>\n");
        ...
        private void RenderCalculateShippingByTotalByPercentForm(StringBuilder html, String EditGUID)
        {
            ...
            html.Append("<form action=\"" + AppLogic.AdminLinkUrl("shipping.aspx") + "?EditGUID=" + EditGUID + "\" method=\"post\" id=\"ByTotalByPercentForm\" name=\"ByTotalByPercentForm\" onsubmit=\"return (validateForm(this) && ByTotalByPercentForm_Validator(this))\" onReset=\"return confirm('Do you want to reset all fields to their starting values?');\">\n");
                    ...
                    using (SqlConnection con = new SqlConnection(DB.GetDBConn()))
                    {
                        ...
                        using (IDataReader rs = DB.GetRS(sqlShipping, con))
                        {
                            while (rs.Read())
                            {
                                ...
                                html.Append("<td align=\"left\"><input class=\"normalButtons\" type=\"Button\" name=\"Delete\" value=\"Delete\" onClick=\"self.location='" + AppLogic.AdminLinkUrl("shipping.aspx") + "?deleteByTotalByPercentid=" + DB.RSFieldGUID(rs, "RowGUID") + "'\"></td>\n");
                                html.Append("</tr>\n");
        ...
        private void RenderUseFixedPriceForm(StringBuilder html, String EditGUID)
        {
            ...
            html.Append("<form action=\"" + AppLogic.AdminLinkUrl("shipping.aspx") + "?EditGUID=" + EditGUID + "\" method=\"post\" id=\"FixedRateForm\" name=\"FixedRateForm\" onsubmit=\"return (validateForm(this) && FixedRateForm_Validator(this))\" onReset=\"return confirm('Do you want to reset all fields to their starting values?');\">\n");
            html.Append("<input type=\"hidden\" name=\"IsSubmitFixedRate\" value=\"true\">\n");
            html.AppendFormat("<input type=\"hidden\" name=\"StoreFilter\" id=\"StoreFilter\" value=\"{0}\" />\n", StoreFilter);

            bool hasShippingMethod = false;
            string query = string.Format("exec aspdnsf_GetStoreShippingMethodMapping  @StoreID = {0}, @IsRTShipping=0, @OnlyMapped = 1", StoreFilter);
        ...
        private void RenderUseFixedPercentageOfTotalForm(StringBuilder html, String EditGUID)
        {
            ...
            html.Append("<form action=\"" + AppLogic.AdminLinkUrl("shipping.aspx") + "?EditGUID=" + EditGUID + "\" method=\"post\" id=\"FixedPercentOfTotalForm\" name=\"FixedPercentOfTotalForm\" onsubmit=\"return (validateForm(this) && FixedPercentOfTotalForm_Validator(this))\" onReset=\"return confirm('Do you want to reset all fields to their starting values?');\">\n");
        ...
        private void RenderCalculateShippingByWeightAndZoneForm(StringBuilder html, String EditGUID)
        {
            ...
            html.Append("<form action=\"" + AppLogic.AdminLinkUrl("shipping.aspx") + "\" method=\"post\" id=\"WeightByZoneSelectForm\" name=\"WeightByZoneSelectForm\" onSubmit=\"return confirm('If you have unsaved changes in your rate table below, click CANCEL and save them first!')\">\n");
            ...
            using (SqlConnection con = new SqlConnection(DB.GetDBConn()))
            {
                ...
                //string queryShippingMethod = string.Format("select * from ShippingMethod   with (NOLOCK)  where StoreID = {0} and IsRTShipping=0 order by DisplayOrder", StoreFilter);
                string queryShippingMethod = string.Format("exec aspdnsf_GetStoreShippingMethodMapping  @StoreID = {0}, @IsRTShipping=0, @OnlyMapped = 0", StoreFilter);
                using (IDataReader rssm = DB.GetRS(queryShippingMethod, con))
                {
                    while (rssm.Read())
                    {
                        html.Append("<option value=\"" + DB.RSFieldInt(rssm, "ShippingMethodID").ToString() + "\" " + CommonLogic.IIF(ShippingMethodID == DB.RSFieldInt(rssm, "ShippingMethodID"), " selected ", "") + ">" + DB.RSFieldByLocale(rssm, "Name", LocaleSetting) + "</option>");
                    }
                }
            }
            ...
            if (ShippingMethodID != 0)
            {
                using (DataTable dtWeightAndZone = new DataTable())
                {
                    ...
                    {
                        html.Append("<form action=\"" + AppLogic.AdminLinkUrl("shipping.aspx") + "\" method=\"post\" id=\"WeightByZoneForm\" name=\"WeightByZoneForm\" onsubmit=\"return (validateForm(this) && WeightByZoneForm_Validator(this))\" onReset=\"return confirm('Do you want to reset all fields to their starting values?');\">\n");
...
//                        }
        ...
        private void RenderCalculateShippingByTotalAndZoneForm(StringBuilder html, String EditGUID)
        {
            ...
            html.Append("<form action=\"" + AppLogic.AdminLinkUrl("shipping.aspx") + "\" method=\"post\" id=\"TotalByZoneSelectForm\" name=\"TotalByZoneSelectForm\" onSubmit=\"return confirm('If you have unsaved changes in your rate table below, click CANCEL and save them first!')\">\n");
            ...
            if (ShippingMethodID != 0)
            {
                using (DataTable dtTotalAndZone = new DataTable())
                {
                    ...
                    {
                        html.Append("<form action=\"" + AppLogic.AdminLinkUrl("shipping.aspx") + "\" method=\"post\" id=\"TotalByZoneForm\" name=\"TotalByZoneForm\" onsubmit=\"return (validateForm(this) && TotalByZoneForm_Validator(this))\" onReset=\"return confirm('Do you want to reset all fields to their starting values?');\">\n");
                        html.Append("<input type=\"hidden\" name=\"EditGUID\" value=\"" + EditGUID + "\">");
                        html.Append("<input type=\"hidden\" name=\"UpdateGUID\" value=\"\">");
                        html.Append("<input type=\"hidden\" name=\"DeleteGUID\" value=\"\">");
                        html.Append("<input type=\"hidden\" name=\"IsSubmitTotalByZone\" value=\"true\">\n");
