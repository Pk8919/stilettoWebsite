...
namespace AspDotNetStorefront
{
    ...
    public partial class checkoutpayment : SkinBase
    {
        ...
        private void InitializePageContent()
        {
            ...
            checkoutheadergraphic.ImageUrl = AppLogic.LocateImageURL("App_Themes/skin_" + SkinID.ToString() + "/images/step_4.gif");
            ...
            if (!(checkoutByAmazon.IsEnabled && checkoutByAmazon.IsCheckingOut))
            {
                ...
                foreach (String PM in AllowedPaymentMethods.Split(','))
                {
                        ...
                        if (GW == Gateway.ro_GWNETAXEPT)
                        {
                            ...
                            if (CommonLogic.QueryStringNativeInt("nexaxepterror") > 0)
                            {
                                ...
                                if (err.Message.IndexOf(AppLogic.GetString("toc.aspx.6", ThisCustomer.SkinID, ThisCustomer.LocaleSetting)) != -1)
                                {
                                    ...
                                    {
                                        lblNetaxeptErrorMsg.Text = err.Message + "<br /><br />";
                    ...
                    else if (PMCleaned == AppLogic.ro_PMPayPalEmbeddedCheckout)
                    {
                        ...
                        if (!IsPostBack)
                        {
                            ctrlPaymentMethod.PAYPALEMBEDDEDCHECKOUTChecked = (BillingAddress.PaymentMethodLastUsed == AppLogic.ro_PMPayPalEmbeddedCheckout || check);
                        }
                    }
                ...
                //paypal advanced custom buisness logic. toggle between credit card form and embedded based on whether or not the cart has recurring items
                if (ctrlPaymentMethod.ShowCREDITCARD && ctrlPaymentMethod.ShowPAYPALEMBEDDEDCHECKOUT)
                {
                    ...
                    else
                    {
                        ...
                        ctrlPaymentMethod.ShowPAYPALEMBEDDEDCHECKOUT = ctrlPaymentMethod.PAYPALEMBEDDEDCHECKOUTChecked = false;
                    }
                }
        ...
        private void ProcessPayment(string PaymentMethod)
        {
            ...
            else if (PM == AppLogic.ro_PMCardinalMyECheck)
            {
                ...
                else
                {
                    ...
                    err = new ErrorMessage(Server.HtmlEncode(AppLogic.GetString("checkoutpayment.aspx.14", ThisCustomer.SkinID, ThisCustomer.LocaleSetting)));
                    Response.Redirect("checkoutpayment.aspx?errormsg=" + err.MessageId);
                }
            }
        ...
        protected void btnContCheckout_Click(object sender, EventArgs e)
        {
            if (RequireTerms && CommonLogic.FormCanBeDangerousContent("TermsAndConditionsRead") == "")
            {
				...
				bool onePageCheckout = CheckOutPageControllerFactory.CreateCheckOutPageController().GetCheckoutType() == CheckOutType.BasicOPC;
				if (onePageCheckout && CommonLogic.QueryStringBool("TermsAndConditionsRead"))
                {
                    if (AppLogic.ProductIsMLExpress() == false)
                    {
