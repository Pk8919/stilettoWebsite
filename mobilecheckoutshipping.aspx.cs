    ...
    public partial class mobilecheckoutshipping : SkinBase
    {
        ...
        private void InitializeShippingMethodDisplayFormat(ShippingMethodCollection shippingMethods)
        {
            foreach (ShippingMethod shipMethod in shippingMethods)
            {
                ...
                {
                    decimal calculatedFreight = AddVatIfApplicable(shipMethod.Freight);
        ...
        public void btnNewShipAddr_OnClick(object sender, System.EventArgs e)
        {
            ...
            {
                ctrlShippingMethods.ErrorMessage += "" + AppLogic.GetString("checkoutshipping.aspx.23", SkinID, ThisCustomer.LocaleSetting) + "<br />";
                foreach (IValidator aValidator in this.Validators)
                {
                    if (!aValidator.IsValid)
                    {
                        ctrlShippingMethods.ErrorMessage += "&bull; " + aValidator.ErrorMessage + "<br />";
                    }
                }
                pnlSelectShipping.Visible = false;
        ...
        private void ShippingDisplay(bool AnyShippingMethodsFound)
        {
            if (cart.CartAllowsShippingMethodSelection)
            {
                ...
                {
                    ctrlShippingMethods.HeaderText = "<p><b>" + String.Format(AppLogic.GetString("checkoutshipping.aspx.15", SkinID, ThisCustomer.LocaleSetting), "checkoutshippingmult.aspx") + "</b></p>";
                }
